      ...
      {isLoading ? (
        ...
        <div className="h-screen w-full">
          <div className="bg-primary/5 w-full h-10 top-0 left-0 p-10 flex justify-between">
            ...
            <div
              className={`group h-12 w-12 border-2 border-primary rounded-full flex justify-center items-center ${isExpanded ? "w-32" : "w-12"} transition-all duration-300 ease-in-out cursor-pointer`}
              onClick={handleDivClick}
            >
              <span className={`font-grifter text-xl ${isExpanded ? "hidden" : "block"}`}>
                <FontAwesomeIcon icon={faRightFromBracket} />
              </span>
              <span
                className={`font-grifter text-md ${isExpanded ? "block" : "hidden"}`}
                onClick={(e) => {
        ...
        <div className="flex flex-col gap-10 h-screen items-center w-full justify-center p-2">
       <Cat/>
